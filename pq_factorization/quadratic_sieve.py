...
def quadratic_factor(n, bound=None):
    ...
    if bound is None: bound = max(int(10 ** (log(n, 10) / 4)), 3)
    ...
    for ai in count(isqrt(n) + 1):
        ...
        for i, p in enumerate(p_list):
            ...
            # not b-smooth
            if not issquare(b2): continue
        
        # if b2 is square, use fermat's method
        if not vi: return ai - isqrt(ai * ai - n)
...
def quadratic_sieve(n, bound=1024, sieve=1000000):
    ...
    for a0 in range(isqrt(n) + 1, isqrt(2 * n), sieve):
        ...
        for ai, r in enumerate(v_list):
            if r != 1 and not issquare(r): continue
            ...
            # if b2 is square, use fermat's method
            if not v: return ai - isqrt(ai * ai - n)
            
            # compare against sieved congruent a values
            for aj in memo[v]:
                x = ai * aj - n
                y = isqrt((ai * ai - n) * (aj * aj - n))
                if 1 < gcd(y - x, n) < n: return gcd(y - x, n)
...
def pc(c_list):
    print('\n'.join(''.join(bin(x)[2:].rjust(5, '0')[i] for x in c_list) for i in range(5)))
            
def create_v():
    global v_list
    v_list = list(map(lambda x: int(x, 2), ['1100', '1101', '0111', '0010', '0001'][::-1]))
