    ...
    while True:
        if n not in sieve:  # n is not a multiple of any of base primes,
            if n < q:  # below next base prime's square, so
                ...
                p = next(bps)
                q = p * p  # pull next base prime, and get its square
        else:
            s = sieve.pop(n)
            nxt = n + s  # n's a multiple of some b.p., find next multiple
            while nxt in sieve:
                nxt += s  # ensure each entry is unique
...
def wheel7(*n):
    ...
    def wheel_prime_pairs():
        yield (11, 0)
        bps = wheel_prime_pairs()  # additional primes supply
        p, pi = next(bps)
        q = p * p  # adv to get 11 sqr'd is 121 as next square to put
        sieve = {}
        n = 13
        ni = 1  #   into sieve dict; init cndidate, wheel ndx
        while True:
            if n not in sieve:  # is not a multiple of previously recorded primes
                if n < q:
                    yield (n, ni)  # n is prime with wheel modulo index
                else:
                    ...
                    sieve[q + p * gaps[pi]] = (
                        p,
                        npi,
                    )  # n == p * p: put next cull position on wheel
                    p, pi = next(bps)
                    q = p * p  # advance next prime and prime square to put
            else:
                s, si = sieve.pop(n)
            ...
            if nni > 47:
                nni = 0
            n += gaps[ni]
            ni = nni  # advance on the wheel

    for p, pi in wheel_prime_pairs():
        yield p  # strip out indexes
